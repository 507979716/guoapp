...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前版本：**0.2.0+6**。
...
已通过 Dart 静态检查、47 项 Flutter 回归、Go 核心竞态回归和 13 项构建镜像 / 同步 / 版本快照测试。合成实测：3 集中仅转换 1 集格式不同的视频；CENC 换封装和 Emby 导出后，删除输入文件，合并成品与导出分集仍完整解码。

已生成 `dist/android/zhenguojian-0.2.0+6-{arm64-v8a,armeabi-v7a,x86_64}.apk`，大小分别约 64.0、76.9、72.0 MiB。三个 APK 均通过版本、电视入口、后台服务声明、升级签名、原生库完整性、ZIP 对齐和 SHA256 核验，与 0.1.4 使用相同签名，可覆盖升级。ARM64 / x86_64 原生库满足 16 KB ELF 对齐；ARMv7 按其 4 KB 平台要求校验。

红果官网联想再次实测“重生”返回 10 条候选；国内 pub 镜像在保留原依赖版本和 SHA256 的情况下通过严格锁文件验证，Gradle 镜像已验证覆盖 APP、插件和 Flutter 内部工程，官方源保留作为备用。镜像开关未用于本次完整 APK 构建，首次完整镜像构建仍以实际依赖可用性为准。

当前 ADB 未连接手机，未运行本轮设备回归，未启动模拟器。Windows Go 核心已交叉编译成功，完整 ZIP 的运行验收需 Windows / Actions；本机缺少 iPhoneOS SDK，iOS 工程及脚本语法检查通过，尚未完成 Xcode 构建和真机验收，也未执行远程 Actions。
